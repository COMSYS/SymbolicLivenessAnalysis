...
// RUN: %klee -output-dir=%t-O0.klee-out -detect-infinite-loops -allocate-determ -stop-after-n-instructions=1000 %t-O0.bc 2>&1 | not FileCheck %s
// RUN: not test -f %t-O0.klee-out/test000001.infty.err

// RUN: rm -rf %t-O1.klee-out
// RUN: %klee -output-dir=%t-O1.klee-out -detect-infinite-loops -allocate-determ -stop-after-n-instructions=10000 %t-O1.bc 2>&1 | FileCheck %s
// RUN: test -f %t-O1.klee-out/test000001.infty.err

// RUN: rm -rf %t-O2.klee-out
// RUN: %klee -output-dir=%t-O2.klee-out -detect-infinite-loops -allocate-determ -stop-after-n-instructions=10000 %t-O2.bc 2>&1 | FileCheck %s
// RUN: test -f %t-O2.klee-out/test000001.infty.err

// RUN: rm -rf %t-O3.klee-out
// RUN: %klee -output-dir=%t-O3.klee-out -detect-infinite-loops -allocate-determ -stop-after-n-instructions=10000 %t-O3.bc 2>&1 | FileCheck %s
