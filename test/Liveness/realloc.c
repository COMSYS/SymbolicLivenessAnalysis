...
// RUN: bash -c 'cat %t-O0.log | grep AB | wc -l | { read linecount; test $linecount == 2; }'
...
// RUN: bash -c 'cat %t-O2.log | grep AB | wc -l | { read linecount; test $linecount == 2; }'
...
// RUN: bash -c 'cat %t-O3.log | grep AB | wc -l | { read linecount; test $linecount == 2; }'
...
int main(int argc, char *argv[]) {
  ...
  // CHECK: KLEE: ERROR: {{[^:]*}}/realloc.c:{{[0-9]+}}: infinite loop{{$}}
  while(1) {
    for(int i = 0; i < 2; i++) {
      str[i] = ('A' + i);
    }
    str[2] = '\0';
    printf("%s\n", str);
    str = realloc(str, 10 * sizeof(char));
  }
}
